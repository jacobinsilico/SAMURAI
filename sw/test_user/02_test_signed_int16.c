...
//
// Test program for signed negative values on user systolic-array accelerator.
...
int main(void)
{
    ...
    printf("Testing systolic array accelerator with negative values\n");
    ...
    *reg32(USER_DESIGN_BASE_ADDR, SA_CTRL_OFFSET) = 0x1;
    ...
    if (errors == 0) {
        printf("Systolic array negative test PASSED\n");
    } else {
        printf("Systolic array negative test FAILED with 0x%x errors\n", errors);
    }
